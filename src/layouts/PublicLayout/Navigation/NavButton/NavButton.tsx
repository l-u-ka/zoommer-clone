...
export default function NavButton({type, text, icon, onClick, onMouseEnter, onMouseLeave, cartItems} : NavItemProps) {
  return (
      <div onClick={onClick} onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave} className={`${type===ButtonEnum.DEFAULT ? "bg-light-theme-bg dark:bg-dark-theme-bg" : "bg-orange-main dark:bg-dark-orange-main"} inline-flex relative h-11 items-center justify-between px-[15px] rounded-xl w-[130px] cursor-pointer opacity-80 transition-all ease-in-out shadow-md duration-300 hover:opacity-100`}>
        ...
        <p className={`${type===ButtonEnum.DEFAULT ? " text-black-main dark:text-dark-black-main" : "text-white"} firago-medium text-sm leading-[17px]`}><FormattedMessage id={text}/></p>
      </div>
  )
}
