...
export default function NavButton({type, text, icon, onClick, onMouseEnter} : NavItemProps) {
  return (
      ...
      <div onClick={onClick} onMouseEnter={onMouseEnter} className={`${type===BUTTON_TYPE_ENUM.DEFAULT ? "bg-white" : "bg-orange-primary"} inline-flex h-full items-center justify-between py-[10px] px-[15px] rounded-xl w-[130px] cursor-pointer ${type===BUTTON_TYPE_ENUM.DEFAULT && "dark:bg-white-400"}`}>
