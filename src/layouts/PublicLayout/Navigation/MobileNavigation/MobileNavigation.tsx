...
export default function MobileNavigation() {
  ...
  function handleNavigation(location: string) {
    if ((authStage !== AuthStageEnum.AUTHORIZED) && (location !== '/')) {
    ...
    <div className="grid grid-cols-3 lg:hidden w-full bg-light-theme-bg dark:bg-[rgba(20,20,20)] fixed left-0 bottom-0 z-20 py-4 border-t border-solid border-b-0 border-x-0 border-white-600">
      <div className="flex justify-center col-span-1 " onClick={() => handleNavigation('/')}>
        <div className="inline-flex justify-center items-center flex-col cursor-pointer">
          ...
          <p className={`${location.pathname === '/' ? 'firago-bold text-orange-main dark:text-dark-orange-main' : 'firago-medium text-gray-main dark:text-dark-gray-main'} text-xs leading-[14px] mt-1`}><FormattedMessage id="home"/></p>
        </div>
      </div>
      <div className="flex justify-center col-span-1" onClick={() => handleNavigation('/profile')}>
        <div className="inline-flex justify-center items-center flex-col cursor-pointer">
          ...
          <p className={`${location.pathname === '/profile' ? 'firago-bold text-orange-main dark:text-dark-orange-main' : 'firago-medium text-gray-main dark:text-dark-gray-main'} text-xs leading-[14px] mt-1`}><FormattedMessage id="profile"/></p>
        </div>
      </div>
      <div className="flex justify-center col-span-1" onClick={() => handleNavigation('/cart')}>
        <div className="inline-flex justify-center items-center flex-col cursor-pointer relative">
          ...
          <p className={`${location.pathname === '/cart' ? 'firago-bold text-orange-main dark:text-dark-orange-main' : 'firago-medium text-gray-main dark:text-dark-gray-main'} text-xs leading-[14px] mt-1`}><FormattedMessage id="cart"/></p>
          {cartItems.length >=1 && <div className="absolute bg-orange-main w-4 h-4 rounded-[50%] top-[-8px] left-[36px] flex justify-center items-center"><p className="text-white firago-medium text-[10px] leading-[14px]">{cartItems.length}</p></div>}
        </div>
      </div>
    </div>
  )
